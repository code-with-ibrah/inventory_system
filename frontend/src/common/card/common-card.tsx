...
import "../../pages/home-page/index/index-page.css"



const CommonCard = (props: any) => {

    const {imgUrl, organiser, title, duration, onClick, className, noHover} = props;
    ...
    const formatDate = (date: any) => {
        ...
        return `${date.getDate()} ${monthsArr[date.getUTCMonth()]} ${date.getUTCFullYear()}`;
    };

    return <React.Fragment>
        <div style={{ cursor: "pointer" }} className={`award-card ${noHover && 'hover-no'}`} onClick={() => clickHandler(this)}>
            <div className="award-img__wrapper">
                {
                    imgUrl ? <img src={imgUrl ? imgUrl : "/src/assets/images/page/award.png"} alt="award image"
                                  className="award-card__img"/>
                        :
                        <svg className={'border-4 border-yellow-500 rounded-t-2xl award-card-no__img'} width="256px" height="256px" viewBox="-2.4 -2.4 28.80 28.80" fill="none"
            ...
            <div className={`award-card__info bg-black-50 ${className && className}`}>
                {title && <Title className="award-title" level={5}>
                    {title.length > 65 ? title.substring(0, 62) + "..." : title}
                </Title>}
                {duration && <p className="award-duration">{formatDate(duration) + ' | Ends: 12:00 am'}</p>}
                {organiser && <p className="award-organiser">{organiser}</p>}
                {props.children}
            </div>
        </div>
    </React.Fragment>
