...
const CategoryForm: React.FC = () => {
    ...
    const onFinish = (values: any) => {
        ...
        setLoading(true);
        ((state?.data && state?.data?.id) ? dispatch(updateCategory({ data: formData, id: state?.data?.id}))
                ...
                <div className={'grid grid-cols-3 md:grid-cols-3 gap-2'}>
                        <Form.Item label="Image">
                                ...
                                <Upload
                                    ...
                                    beforeUpload={() => false}
                                    onChange={async (file) => await handleImageChange(file, setImageFile, setThumbnailFile)}>
                                    {
