...
const UserForm: React.FC = () => {
    ...
    const onFinish = (values: any) => {
        ...
        // @ts-ignore
        values.organisationId = user?.organisation?.id;
        values.roleId = user?.roleId;
        values.code = user?.code;

        if(!values.organisationId){
            setLoading(false);
            TlaError("Account creation is restricted to organizational accounts.");
            navigate(-1);
            return;
        }

        setLoading(true);
        ((state?.data && state?.data?.id) ? dispatch(updateUser({ data: values, id: state?.data?.id}))
